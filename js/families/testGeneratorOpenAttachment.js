/*
 * Generador de tests para acciones de abrir adjuntos (Open Attachment)
 * Maneja configuraciones específicas para navegación de URLs y botones de aceptar/cancelar
 */

import { BaseTestGenerator } from "../baseTestGenerator.js"

/**
 * Clase que genera tests para acciones de abrir adjuntos
 * Incluye campos específicos para URLs, navegación y botones de confirmación
 */
export class TestGeneratorOpenAttachment extends BaseTestGenerator {
    /**
     * Constructor del generador de tests de abrir adjuntos
    ...
    /**
     * Crea el formulario para configurar tests de abrir adjuntos
     * Incluye campos específicos para URL y posición, con valores fijos para navegación y botones
    ...
    createForm() {
      ...
      const formHTML = `
              ...
              <div class="alert alert-info">
                  <i class="fas fa-info-circle"></i> Configuración de Test para acción de Abrir Adjunto
              </div>
              ...
              <!-- Sección específica para configuración de adjuntos -->
              <div class="form-section">
                  <h4><i class="fas fa-paperclip"></i> Configuración de Adjunto</h4>
                  
                  <!-- Campo editable para la posición de la URL -->
                  <div class="form-group">
                      <label for="positionURL">Posición de la URL:</label>
                      <input type="text" id="positionURL" value="0" data-json-path="positionURL" />
                  </div>
  
                  <!-- Campo editable para la URL -->
                  <div class="form-group">
                      <label for="url">URL:</label>
                      <input type="text" id="url" value="TRAZiT" data-json-path="url" />
                  </div>
              </div>
    ...
    /**
     * Configura todos los event listeners para el formulario de abrir adjuntos
     ...
     * Recopila todos los datos del formulario y genera el JSON de test específico para adjuntos
     */
    updatePreview() {
      ...
      const formData = {
        ...
        // Campos específicos de adjuntos
        positionURL: document.getElementById("positionURL") ? document.getElementById("positionURL").value : "0",
        url: document.getElementById("url") ? document.getElementById("url").value : "TRAZiT",
      }
  
      // Generar los datos de test y actualizar la vista previa
      const testData = this.generateOpenAttachmentTestData(formData)
    ...
    generateOpenAttachmentTestData(formData) {
      ...
      // Agregar campos específicos de abrir adjuntos
      testData.positionURL = formData.positionURL
      testData.url = formData.url
      testData.phraseNavigation = "URL Navigation"
      testData.screenShotsUrl = "URL"
      testData.buttonAccept = "Accept"
      testData.buttonCancel = "Cancel"
      testData.screenShotsAccept = "Accept"
      testData.screenShotsCancel = "Cancel"
